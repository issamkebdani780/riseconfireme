                    ...
                    <div className="w-full lg:w-1/2 space-y-12">
                        <div className="space-y-6">
                            <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-blue-900/20 rounded-full border border-blue-100 dark:border-blue-800 shadow-sm transition-colors mx-auto lg:mx-0">
                                ...
                                <span className="text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest leading-none">{t('Qui sommes-nous ?')}</span>
                            </div>
                            <h2 className="text-4xl lg:text-5xl font-extrabold text-heading dark:text-white leading-tight text-start">
                                {t('La voix de')} <span className="text-primary italic">{t('votre succès')}</span> {t('e-commerce.')}
                            </h2>
                            <p className="text-lg text-body dark:text-slate-400 leading-relaxed font-medium text-start">
                                {t('apropos_desc')}
                            </p>
                        </div>
                        ...
                        <div className="grid sm:grid-cols-2 gap-8">
                            {values.map((item, i) => (
                                <div key={i} className="space-y-4 group text-start">
                                    <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-900 flex items-center justify-center text-primary border border-slate-100 dark:border-slate-800 group-hover:scale-110 transition-transform mx-0">
                                        {item.icon}
                                    </div>
