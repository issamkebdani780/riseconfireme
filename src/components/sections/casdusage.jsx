...
const UseCases = () => {
    ...
    const cases = [
        {
            title: t("Débutant E-commerce"),
            problem: t("prob_beginner"),
            solution: t("sol_beginner"),
            benefit: t("benefit_beginner"),
        ...
        {
            title: t("Boutique en Croissance"),
            problem: t("prob_growth"),
            solution: t("sol_growth"),
            benefit: t("benefit_growth"),
        ...
        {
            title: t("Marketplace & Marques"),
            problem: t("prob_market"),
            solution: t("sol_market"),
            benefit: t("benefit_market"),
            icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
            color: "text-violet-600",
            bg: "bg-violet-50",
            cta: t("Mise à l'échelle")
        },
        {
            title: t("Social Sellers"),
            problem: t("prob_social"),
            solution: t("sol_social"),
            benefit: t("benefit_social"),
        ...
        {
            title: t("Secteur Logistique"),
            problem: t("prob_log"),
            solution: t("sol_log"),
            benefit: t("benefit_log"),
        ...
        {
            title: t("Fintech & Services"),
            problem: t("prob_fintech"),
            solution: t("sol_fintech"),
            benefit: t("benefit_fintech"),
            ...
            <div className="container mx-auto px-4 sm:px-6">
                ...
                <div className="max-w-3xl mx-auto text-center mb-20 space-y-6">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 dark:bg-slate-900 rounded-full border border-slate-100 dark:border-slate-800">
                        ...
                        <span className="text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest leading-none">{t("Cas d'usage")}</span>
                    </div>
                    <h2 className="text-3xl lg:text-5xl font-extrabold text-heading dark:text-white leading-tight animate-slide-up">
                        {t("Adapté à")} <span className="text-primary italic">{t("votre business")}</span>
                    </h2>
                    ...
                    {cases.map((useCase, i) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <h3 className="text-lg font-bold text-heading dark:text-white leading-tight text-start">{useCase.title}</h3>
                            </div>

                            <div className="space-y-5 flex-1 mb-6 mt-2">
                                <div className="space-y-1.5">
                                    <div className="text-[10px] font-extrabold text-red-500/80 uppercase tracking-widest flex items-center gap-1.5 text-start">{t("Problème")}</div>
                                    <p className="text-[14px] text-slate-500 dark:text-slate-400 font-medium leading-snug text-start">{useCase.problem}</p>
                                </div>

                                <div className="space-y-1.5">
                                    <div className="text-[10px] font-extrabold text-emerald-500 uppercase tracking-widest flex items-center gap-1.5 text-start">{t("Solution")}</div>
                                    <p className="text-[14px] text-slate-800 dark:text-slate-200 font-bold leading-snug text-start">{useCase.solution}</p>
                                </div>
                            </div>

                             <div className="mt-2 pt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between mb-8">
                                <div className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t("Gain Direct")}</div>
                             ...
                             <button className={`w-full group/btn inline-flex justify-center items-center gap-2 px-4 py-3 rounded-2xl text-sm font-bold transition-all duration-300 ${useCase.recommended ? 'bg-primary text-white hover:bg-primary-hover shadow-md' : 'bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-100 dark:border-slate-700'}`}>
                                {useCase.cta}
                                <svg className="w-4 h-4 group-hover/btn:translate-x-1 rtl:group-hover/btn:-translate-x-1 rtl:rotate-180 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
